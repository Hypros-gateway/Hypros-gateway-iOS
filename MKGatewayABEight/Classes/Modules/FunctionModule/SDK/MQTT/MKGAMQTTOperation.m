...
/**
 线程ID
 */
@property (nonatomic, assign)mk_ga_serverOperationID operationID;
...
@implementation MKGAMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKGAMQTTOperation销毁");
}
...
#pragma mark - MKGAMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKGAMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_ga_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_ga_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
