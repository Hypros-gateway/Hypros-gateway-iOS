//
//  MKGAFilterByAdvNameController.m
...
@interface MKGAFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGAFilterByAdvNameModel *dataModel;

@end

@implementation MKGAFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKGAFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKGAFilterEditSectionHeaderViewModel *headerModel = [[MKGAFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAFilterByAdvNameController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
