...
@interface MKGAFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGAFilterByMacModel *dataModel;

@end

@implementation MKGAFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAFilterByMacController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByMacModel alloc] init];
    }
    return _dataModel;
}
