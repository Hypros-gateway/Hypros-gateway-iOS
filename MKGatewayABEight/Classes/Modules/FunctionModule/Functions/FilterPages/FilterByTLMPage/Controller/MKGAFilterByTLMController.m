...
@interface MKGAFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGAFilterByTLMModel *dataModel;

@end

@implementation MKGAFilterByTLMController

- (void)dealloc {
    NSLog(@"MKGAFilterByTLMController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAFilterByTLMController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
