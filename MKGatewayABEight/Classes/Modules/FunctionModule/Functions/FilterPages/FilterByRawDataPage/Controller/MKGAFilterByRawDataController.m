//
//  MKGAFilterByRawDataController.m
...
@interface MKGAFilterByRawDataController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGAFilterByRawDataModel *dataModel;

@end

@implementation MKGAFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKGAFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKGAFilterByBeaconController *vc = [[MKGAFilterByBeaconController alloc] init];
        vc.pageType = mk_ga_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKGAFilterByUIDController *vc = [[MKGAFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKGAFilterByURLController *vc = [[MKGAFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKGAFilterByTLMController *vc = [[MKGAFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //MKiBeacon
        MKGAFilterByBeaconController *vc = [[MKGAFilterByBeaconController alloc] init];
        vc.pageType = mk_ga_filterByBeaconPageType_MKBeacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 5) {
        //MKiBeacon&ACC
        MKGAFilterByBeaconController *vc = [[MKGAFilterByBeaconController alloc] init];
        vc.pageType = mk_ga_filterByBeaconPageType_MKBeaconAcc;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //Other
        MKGAFilterByOtherController *vc = [[MKGAFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configFilterBXPACC:(BOOL)isOn {
    ...
    [MKGAMQTTInterface ga_configFilterBXPACC:isOn
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGAMQTTInterface ga_configFilterBXPTH:isOn
                                     deviceID:[MKGADeviceModeManager shared].deviceID
                                   macAddress:[MKGADeviceModeManager shared].macAddress
                                        topic:[MKGADeviceModeManager shared].subscribedTopic
                                     sucBlock:^(id  _Nonnull returnData) {
...
- (MKGAFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
