//
//  MKGAFilterByBeaconController.m
...
@interface MKGAFilterByBeaconController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKGAFilterNormalTextFieldCellDelegate,
MKGAFilterBeaconCellDelegate>
...
@property (nonatomic, strong)MKGAFilterByBeaconModel *dataModel;

@end

@implementation MKGAFilterByBeaconController

- (void)dealloc {
    NSLog(@"MKGAFilterByBeaconController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGAFilterNormalTextFieldCell *cell = [MKGAFilterNormalTextFieldCell initCellWithTableView:tableView];
        cell.dataModel = self.section1List[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    MKGAFilterBeaconCell *cell = [MKGAFilterBeaconCell initCellWithTableView:tableView];
...
#pragma mark - MKGAFilterNormalTextFieldCellDelegate
- (void)mk_ga_filterNormalTextFieldValueChanged:(NSString *)text index:(NSInteger)index {
    if (index == 0) {
        //iBeacon UUID
        self.dataModel.uuid = text;
        MKGAFilterNormalTextFieldCellModel *cellModel = self.section1List[0];
        cellModel.textFieldValue = text;
        return;
    }
}

#pragma mark - MKGAFilterBeaconCellDelegate
- (void)mk_ga_beaconMinValueChanged:(NSString *)value index:(NSInteger)index {
    MKGAFilterBeaconCellModel *cellModel = self.section2List[index];
...
- (void)loadSection1Datas {
    MKGAFilterNormalTextFieldCellModel *cellModel = [[MKGAFilterNormalTextFieldCellModel alloc] init];
    cellModel.index = 0;
    if (self.pageType == mk_ga_filterByBeaconPageType_beacon) {
...
- (void)loadSection2Datas {
    MKGAFilterBeaconCellModel *cellModel1 = [[MKGAFilterBeaconCellModel alloc] init];
    cellModel1.index = 0;
    cellModel1.minValue = self.dataModel.minMajor;
    cellModel1.maxValue = self.dataModel.maxMajor;
    [self.section2List addObject:cellModel1];
    
    MKGAFilterBeaconCellModel *cellModel2 = [[MKGAFilterBeaconCellModel alloc] init];
    cellModel2.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAFilterByBeaconController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAFilterByBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByBeaconModel alloc] initWithPageType:self.pageType];
    }
    return _dataModel;
}
