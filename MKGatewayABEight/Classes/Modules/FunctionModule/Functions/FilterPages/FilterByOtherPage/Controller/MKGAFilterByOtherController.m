...
@interface MKGAFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKGAFilterEditSectionHeaderViewDelegate,
MKGAFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKGAFilterByOtherModel *dataModel;

@end

@implementation MKGAFilterByOtherController

- (void)dealloc {
    NSLog(@"MKGAFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKGAFilterEditSectionHeaderView *headerView = [MKGAFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKGAFilterEditSectionHeaderViewModel *model = [[MKGAFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGAFilterByRawDataCell *cell = [MKGAFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKGAFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_ga_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKGAFilterByRawDataCellModel *cellModel = [[MKGAFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKGAFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)ga_rawFilterDataChanged:(mk_ga_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKGAFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_ga_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_ga_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_ga_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_ga_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKGAFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKGAFilterRawAdvDataModel *model = [[MKGAFilterRawAdvDataModel alloc] init];
...
- (mk_ga_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_ga_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_ga_filterByOther_AB;
        }
        return mk_ga_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_ga_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_ga_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_ga_filterByOther_AOrBOrC;
        }
    }
    return mk_ga_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAFilterByOtherController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
