...
@interface MKGAAdvBeaconController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKGAAdvBeaconTxPowerCellDelegate>
...
@property (nonatomic, strong)MKGAAdvBeaconModel *dataModel;

@end

@implementation MKGAAdvBeaconController

- (void)dealloc {
    NSLog(@"MKGAAdvBeaconController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKGAAdvBeaconTxPowerCell *cell = [MKGAAdvBeaconTxPowerCell initCellWithTableView:tableView];
...
#pragma mark - MKGAAdvBeaconTxPowerCellDelegate
- (void)mk_advBeaconTxPowerCell_measurePowerValueChanged:(NSString *)measurePower {
    self.dataModel.rssi = [measurePower integerValue];
    
    MKGAAdvBeaconTxPowerCellModel *cellModel = self.section2List[0];
    cellModel.measurePower = self.dataModel.rssi;
}
...
- (void)loadSection2Datas {
    MKGAAdvBeaconTxPowerCellModel *cellModel = [[MKGAAdvBeaconTxPowerCellModel alloc] init];
    cellModel.measurePower = self.dataModel.rssi;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAAdvBeaconController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
