...
@implementation MKGAFilterCellModel
@end

@interface MKGAFilterCell ()
...
@implementation MKGAFilterCell

+ (MKGAFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKGAFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGAFilterCellIdenty"];
    if (!cell) {
        cell = [[MKGAFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGAFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ga_filterValueChanged:index:)]) {
            [self.delegate ga_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGAFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAFilterCellModel.class]) {
        return;
    }
