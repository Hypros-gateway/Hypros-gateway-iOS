...
@interface MKGAUploadOptionController ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKGAFilterCellDelegate>
...
@property (nonatomic, strong)MKGAUploadOptionModel *dataModel;

@end

@implementation MKGAUploadOptionController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGAFilterCell *cell = [MKGAFilterCell initCellWithTableView:tableView];
...
- (void)ga_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKGAFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKGAFilterByMacController *vc = [[MKGAFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKGAFilterByAdvNameController *vc = [[MKGAFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKGAFilterByRawDataController *vc = [[MKGAFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKGADuplicateDataFilterController *vc = [[MKGADuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKGAUploadDataOptionController *vc = [[MKGAUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKGAFilterCellModel *cellModel = [[MKGAFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAUploadOptionController", @"ga_saveIcon.png")
...
- (MKGAUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAUploadOptionModel alloc] init];
    }
    return _dataModel;
}
