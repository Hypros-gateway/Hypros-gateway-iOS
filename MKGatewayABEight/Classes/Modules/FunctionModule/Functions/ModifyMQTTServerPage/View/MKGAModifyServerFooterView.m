...
@implementation MKGAModifyServerFooterViewModel
@end
...
@interface MKGAModifyServerFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGAModifyServerSSLViewDelegate,
MKGAModifyServerSettingViewDelegate>
...
@property (nonatomic, strong)MKGAModifyServerSSLView *sslView;

@property (nonatomic, strong)MKGAModifyServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKGAModifyServerFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_qosChanged:)]) {
        [self.delegate ga_mqtt_modifyMQTT_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKGAModifyServerSSLViewDelegate
- (void)ga_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)ga_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_certificateChanged:)]) {
        [self.delegate ga_mqtt_modifyMQTT_certificateChanged:certificate];
    }
}
...
- (void)ga_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:value textID:(index + 5)];
    }
}

#pragma mark - MKGAModifyServerSettingViewDelegate
- (void)ga_mqtt_modifyDevice_wifiSSIDChanged:(NSString *)ssid {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:ssid textID:3];
    }
}

- (void)ga_mqtt_modifyDevice_wifiPasswordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate ga_mqtt_modifyMQTT_textFieldValueChanged:password textID:4];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKGAModifyServerFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAModifyServerFooterViewModel.class]) {
        return;
    }
    ...
    MKGAModifyServerSSLViewModel *sslModel = [[MKGAModifyServerSSLViewModel alloc] init];
    ...
    MKGAModifyServerSettingViewModel *settingModel = [[MKGAModifyServerSettingViewModel alloc] init];
...
- (MKGAModifyServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGAModifyServerSSLView alloc] init];
...
- (MKGAModifyServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKGAModifyServerSettingView alloc] init];
