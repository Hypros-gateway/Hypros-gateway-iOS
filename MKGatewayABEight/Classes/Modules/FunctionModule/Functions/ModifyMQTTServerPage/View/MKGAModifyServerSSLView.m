...
@implementation MKGAModifyServerSSLViewModel
@end

@interface MKGAModifyServerSSLView ()<MKGAModifyServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKGAModifyServerSSLTextField *hostView;

@property (nonatomic, strong)MKGAModifyServerSSLTextField *portView;

@property (nonatomic, strong)MKGAModifyServerSSLTextField *caFileView;

@property (nonatomic, strong)MKGAModifyServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKGAModifyServerSSLTextField *clientCertView;

@end

@implementation MKGAModifyServerSSLView
...
#pragma mark - MKGAModifyServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)ga_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate ga_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate ga_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate ga_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGAModifyServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAModifyServerSSLViewModel.class]) {
        return;
    }
    ...
    MKGAModifyServerSSLTextFieldModel *hostModel = [[MKGAModifyServerSSLTextFieldModel alloc] init];
    hostModel.index = 0;
    hostModel.msg = @"Host";
    ...
    MKGAModifyServerSSLTextFieldModel *portModel = [[MKGAModifyServerSSLTextFieldModel alloc] init];
    portModel.index = 1;
    portModel.msg = @"Port";
    ...
    MKGAModifyServerSSLTextFieldModel *caModel = [[MKGAModifyServerSSLTextFieldModel alloc] init];
    caModel.index = 2;
    ...
    MKGAModifyServerSSLTextFieldModel *clientKeyModel = [[MKGAModifyServerSSLTextFieldModel alloc] init];
    ...
    MKGAModifyServerSSLTextFieldModel *clientModel = [[MKGAModifyServerSSLTextFieldModel alloc] init];
    clientModel.index = 4;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayABEight", @"MKGAModifyServerSSLView", @"ga_switchSelectedIcon.png") : LOADICON(@"MKGatewayABEight", @"MKGAModifyServerSSLView", @"ga_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAModifyServerSSLView", @"ga_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKGAModifyServerSSLTextField *)hostView {
    if (!_hostView) {
        _hostView = [[MKGAModifyServerSSLTextField alloc] init];
        _hostView.delegate = self;
    }
    return _hostView;
}

- (MKGAModifyServerSSLTextField *)portView {
    if (!_portView) {
        _portView = [[MKGAModifyServerSSLTextField alloc] init];
        _portView.delegate = self;
    }
    return _portView;
}

- (MKGAModifyServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKGAModifyServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKGAModifyServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKGAModifyServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKGAModifyServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKGAModifyServerSSLTextField alloc] init];
