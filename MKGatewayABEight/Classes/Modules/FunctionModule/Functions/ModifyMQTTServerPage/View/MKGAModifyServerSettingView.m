...
@implementation MKGAModifyServerSettingViewModel
@end

@interface MKGAModifyServerSettingView ()
...
@implementation MKGAModifyServerSettingView
...
#pragma mark - setter
- (void)setDataModel:(MKGAModifyServerSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAModifyServerSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)ssidField {
    if (!_ssidField) {
        ...
        _ssidField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyDevice_wifiSSIDChanged:)]) {
                [self.delegate ga_mqtt_modifyDevice_wifiSSIDChanged:text];
            }
        };
...
- (MKTextField *)passwordField {
    if (!_passwordField) {
        ...
        _passwordField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ga_mqtt_modifyDevice_wifiPasswordChanged:)]) {
                [self.delegate ga_mqtt_modifyDevice_wifiPasswordChanged:text];
            }
        };
