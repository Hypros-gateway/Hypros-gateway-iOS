...
@interface MKGAModifyServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGAModifyServerFooterViewDelegate>
...
@property (nonatomic, strong)MKGAModifyServerModel *dataModel;

@property (nonatomic, strong)MKGAModifyServerFooterView *sslParamsView;

@property (nonatomic, strong)MKGAModifyServerFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKGAModifyServerController

- (void)dealloc {
    NSLog(@"MKGAModifyServerController销毁");
...
#pragma mark - MKGAModifyServerFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)ga_mqtt_modifyMQTT_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)ga_mqtt_modifyMQTT_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)ga_mqtt_modifyMQTT_qosChanged:(NSInteger)qos {
...
- (void)ga_mqtt_modifyMQTT_certificateChanged:(NSInteger)certificate {
...
- (void)updateMQTTServer {
    ...
    @weakify(self);
    [self.dataModel updateServerWithDeviceID:[MKGADeviceModeManager shared].deviceID
                                  macAddress:[MKGADeviceModeManager shared].macAddress
                                  deviceName:[MKGADeviceModeManager shared].deviceName
...
- (void)updateLocal {
    ...
    deviceModel.deviceType = [MKGADeviceModeManager shared].deviceType;
    deviceModel.deviceID = [MKGADeviceModeManager shared].deviceID;
    deviceModel.deviceName = [MKGADeviceModeManager shared].deviceName;
    deviceModel.macAddress = [MKGADeviceModeManager shared].macAddress;
    ...
    deviceModel.onLineState = MKGADeviceModelStateOffline;
    
    [MKGADeviceDatabaseManager insertDeviceList:@[deviceModel] sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)popAction:(MKGADeviceModel *)deviceModel {
    [self popToViewControllerWithClassName:@"MKGADeviceListController"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ga_addNewDeviceSuccessNotification"
                                                        object:nil
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAModifyServerController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAModifyServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAModifyServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGAModifyServerFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGAModifyServerFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKGAModifyServerFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGAModifyServerFooterViewModel alloc] init];
    }
