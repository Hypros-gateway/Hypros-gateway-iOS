//
//  MKGAServerConfigDeviceSettingView.m
...
@implementation MKGAServerConfigDeviceSettingViewModel
@end

@interface MKGAServerConfigDeviceSettingView ()
...
@implementation MKGAServerConfigDeviceSettingView
...
- (void)timeZoneButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_deviecSetting_timeZoneChanged:)]) {
            [self.delegate ga_mqtt_deviecSetting_timeZoneChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGAServerConfigDeviceSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAServerConfigDeviceSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)deviceIDField {
    if (!_deviceIDField) {
        ...
        _deviceIDField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ga_mqtt_deviecSetting_deviceIDChanged:)]) {
                [self.delegate ga_mqtt_deviecSetting_deviceIDChanged:text];
            }
        };
...
- (MKTextField *)ntpUrlField {
    if (!_ntpUrlField) {
        ...
        _ntpUrlField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ga_mqtt_deviecSetting_ntpURLChanged:)]) {
                [self.delegate ga_mqtt_deviecSetting_ntpURLChanged:text];
            }
        };
