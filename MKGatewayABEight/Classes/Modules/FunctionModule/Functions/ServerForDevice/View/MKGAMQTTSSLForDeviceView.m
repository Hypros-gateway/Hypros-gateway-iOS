...
@implementation MKGAMQTTSSLForDeviceViewModel
@end

@interface MKGAMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKGAMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate ga_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate ga_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate ga_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate ga_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate ga_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGAMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForDeviceView", @"ga_switchSelectedIcon.png") : LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForDeviceView", @"ga_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForDeviceView", @"ga_switchUnselectedIcon.png") forState:UIControlStateNormal];
