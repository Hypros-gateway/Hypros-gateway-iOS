//
//  MKGAServerForDeviceController.m
...
@interface MKGAServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGAServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKGAServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKGAServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKGAServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@property (nonatomic, strong)MKGAWifiAlertView *alertView;
...
@implementation MKGAServerForDeviceController

- (void)dealloc {
    NSLog(@"MKGAServerForDeviceController销毁");
...
- (void)rightButtonMethod {
    ...
    self.alertView = nil;
    self.alertView = [[MKGAWifiAlertView alloc] init];
    @weakify(self);
...
#pragma mark - MKGAServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)ga_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)ga_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)ga_mqtt_serverForDevice_qosChanged:(NSInteger)qos {
...
- (void)ga_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)ga_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)ga_mqtt_serverForDevice_timeZoneChanged:(NSInteger)timeZone {
...
- (void)connectStatusChanged {
    if ([MKGACentralManager shared].connectStatus == mk_ga_centralConnectStatusConnected) {
        return;
    }
    if (self.alertView) {
...
- (void)receiveDeviceNetState:(NSNotification *)note {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKGAReceiveDeviceNetStateNotification
                                                  object:nil];
    if (self.connectTimer) {
        dispatch_cancel(self.connectTimer);
    }
    ...
    [self.dataModel readPramsFromDeviceWithSucBlock:^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(connectStatusChanged)
                                                     name:mk_ga_peripheralConnectStateChangedNotification
                                                   object:nil];
...
- (void)configServerParamsComplete {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                  ...
                                                  object:nil];
    NSString *topic = @"";
    if (ValidStr([MKGAMQTTManager shared].subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKGAMQTTManager shared].subscribeTopic;
    }else {
        topic = self.dataModel.publishTopic;
    }
    [self.progressView show];
    [[MKGAMQTTManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKGADeviceModel *deviceModel = [[MKGADeviceModel alloc] init];
    ...
    MKGAConnectSuccessController *vc = [[MKGAConnectSuccessController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAServerForDeviceController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGAServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGAServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKGAServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGAServerConfigDeviceFooterViewModel alloc] init];
    }
