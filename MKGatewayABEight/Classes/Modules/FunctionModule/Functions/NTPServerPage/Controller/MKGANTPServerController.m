...
@interface MKGANTPServerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGANTPServerModel *dataModel;

@end

@implementation MKGANTPServerController

- (void)dealloc {
    NSLog(@"MKGANTPServerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGANTPServerController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGANTPServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGANTPServerModel alloc] init];
    }
    return _dataModel;
}
