//
//  MKGADuplicateDataFilterModel.m
...
@interface MKGADuplicateDataFilterModel ()
...
@implementation MKGADuplicateDataFilterModel
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKGAMQTTInterface ga_configDuplicateDataFilter:self.rule period:[self.time longLongValue] deviceID:[MKGADeviceModeManager shared].deviceID macAddress:[MKGADeviceModeManager shared].macAddress topic:[MKGADeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
