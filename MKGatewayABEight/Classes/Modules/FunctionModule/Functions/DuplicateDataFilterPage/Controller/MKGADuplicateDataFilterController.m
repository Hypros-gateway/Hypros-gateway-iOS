//
//  MKGADuplicateDataFilterController.m
...
@interface MKGADuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGADuplicateDataFilterModel *dataModel;

@end

@implementation MKGADuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKGADuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGADuplicateDataFilterController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGADuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGADuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
