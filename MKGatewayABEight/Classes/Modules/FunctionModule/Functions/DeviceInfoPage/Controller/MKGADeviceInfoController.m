...
@interface MKGADeviceInfoController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKGADeviceInfoModel *dataModel;

@end

@implementation MKGADeviceInfoController

- (void)dealloc {
    NSLog(@"MKGADeviceInfoController销毁");
}
...
- (MKGADeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGADeviceInfoModel alloc] init];
    }
    return _dataModel;
}
