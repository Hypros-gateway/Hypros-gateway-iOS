...
@interface MKGAOTAController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGASlaveFileSelectControllerDelegate>
...
@property (nonatomic, strong)MKGAOTADataModel *dataModel;

@property (nonatomic, strong)UIButton *typeButton;

@property (nonatomic, strong)MKGADFUModel *dfuModel;

@end

@implementation MKGAOTAController

- (void)dealloc {
    NSLog(@"MKGAOTAController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.dataModel.type == 1) {
        //Slave Firmware
        MKGADFUFileCell *cell = [MKGADFUFileCell initCellWithTableView:tableView];
...
#pragma mark - MKGADFUFileCellDelegate
- (void)ga_dfuFileButtonPressed:(NSInteger)index {
    if (index != 0) {
        return;
    }
    MKGASlaveFileSelectController *vc = [[MKGASlaveFileSelectController alloc] init];
    vc.delegate = self;
    [self.navigationController pushViewController:vc animated:YES];
}

#pragma mark - MKGASlaveFileSelectControllerDelegate
- (void)ga_slaveFileSelected:(NSString *)fileName {
    self.dataModel.slaveModel.fileName = fileName;
    MKGADFUFileCellModel *cellModel = self.slaveList[0];
...
- (void)receiveOTAResult:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"device_info"][@"device_id"])
        || ![[MKGADeviceModeManager shared].deviceID isEqualToString:user[@"device_info"][@"device_id"]]) {
        return;
    }
...
- (void)otaMasterFirmware {
    ...
    [MKGAMQTTInterface ga_otaMasterFirmware:self.dataModel.masterModel.host
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveOTAResult:)
                                                     name:MKGAReceiveDeviceOTAResultNotification
                                                   object:nil];
    }
...
- (void)otaSlaveFirmware {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGAMQTTInterface ga_updateSlaveFirmwareWithDeviceID:[MKGADeviceModeManager shared].deviceID
                                                 macAddress:[MKGADeviceModeManager shared].macAddress
                                                      topic:[MKGADeviceModeManager shared].subscribedTopic
                                                   sucBlock:^(id  _Nonnull returnData) {
        NSInteger resultCode = [returnData[@"data"][@"ota_state"] integerValue];
...
- (void)otaCACertificate {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGAMQTTInterface ga_otaCACertificate:self.dataModel.caFileModel.host
...
- (void)otaSelfSignedServerCertificates {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKGAMQTTInterface ga_otaSelfSignedCertificates:self.dataModel.signedModel.host
...
- (void)readDataFromDevice {
    ...
    [MKGAMQTTInterface ga_readSlaveDeviceInfoWithDeviceID:[MKGADeviceModeManager shared].deviceID
                                                 macAddress:[MKGADeviceModeManager shared].macAddress
                                                      topic:[MKGADeviceModeManager shared].subscribedTopic
                                                   sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)loadSlaveDatas {
    MKGADFUFileCellModel *cellModel = [[MKGADFUFileCellModel alloc] init];
    cellModel.index = 0;
...
- (MKGAOTADataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAOTADataModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGADFUModel *)dfuModel {
    if (!_dfuModel) {
        _dfuModel = [[MKGADFUModel alloc] init];
    }
    return _dfuModel;
}
