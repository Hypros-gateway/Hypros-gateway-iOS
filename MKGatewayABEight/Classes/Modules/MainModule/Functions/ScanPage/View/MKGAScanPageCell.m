...
@implementation MKGAScanPageCell

+ (MKGAScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKGAScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGAScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKGAScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGAScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKGAScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAScanPageModel.class]) {
        return;
    }
