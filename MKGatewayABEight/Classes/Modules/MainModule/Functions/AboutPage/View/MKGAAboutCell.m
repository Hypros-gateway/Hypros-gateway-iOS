...
@implementation MKGAAboutCellModel
@end

@interface MKGAAboutCell ()
...
@implementation MKGAAboutCell

+ (MKGAAboutCell *)initCellWithTableView:(UITableView *)table {
    MKGAAboutCell *cell = [table dequeueReusableCellWithIdentifier:@"MKGAAboutCellIdenty"];
    if (!cell) {
        cell = [[MKGAAboutCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGAAboutCellIdenty"];
    }
    return cell;
}
...
#pragma mark -
- (void)setDataModel:(MKGAAboutCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAAboutCellModel.class]) {
        return;
    }
    self.typeLabel.text = _dataModel.typeMessage;
    self.valueLabel.text = _dataModel.value;
    self.icon.image = LOADICON(@"MKGatewayABEight", @"MKGAAboutCell", _dataModel.iconName);
