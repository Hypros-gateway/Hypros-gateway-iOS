...
@implementation MKGAMQTTSSLForAppViewModel
@end

@interface MKGAMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKGAMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate ga_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate ga_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate ga_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ga_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate ga_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGAMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAMQTTSSLForAppViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForAppView", @"ga_switchSelectedIcon.png") : LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForAppView", @"ga_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAMQTTSSLForAppView", @"ga_switchUnselectedIcon.png") forState:UIControlStateNormal];
