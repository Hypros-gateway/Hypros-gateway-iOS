...
@interface MKGAServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGAServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKGAServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKGAServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKGAServerForAppController

- (void)dealloc {
    NSLog(@"MKGAServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKGAMQTTServerManager shared] saveServerParams:self.dataModel];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(serverManagerStateChanged)
                                                 name:MKGAMQTTSessionManagerStateChangedNotification
                                               object:nil];
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    self.leftButton.enabled = NO;
    [[MKGAMQTTServerManager shared] connect];
}
...
- (void)leftButtonMethod {
    ...
    [alertView showAlertWithTitle:@"" message:msg notificationName:@"mk_ga_needDismissAlert"];
}
...
#pragma mark - MKGAServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)ga_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)ga_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)ga_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)ga_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)ga_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)serverManagerStateChanged {
    if ([MKGAMQTTServerManager shared].state == MKMQTTSessionManagerStateConnecting) {
        [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
        [self startReceiveTimer];
        self.leftButton.enabled = NO;
        return;
    }
    if ([MKGAMQTTServerManager shared].state == MKMQTTSessionManagerStateConnected) {
    ...
    if ([MKGAMQTTServerManager shared].state == MKMQTTSessionManagerStateError) {
        [self connectFailed];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayABEight", @"MKGAServerForAppController", @"ga_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGAServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGAServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGAServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGAServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKGAServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGAServerConfigAppFooterViewModel alloc] init];
    }
