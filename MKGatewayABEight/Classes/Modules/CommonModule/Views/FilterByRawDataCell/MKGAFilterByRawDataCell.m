...
@implementation MKGAFilterByRawDataCellModel
...
@interface MKGAFilterByRawDataCell ()
...
@implementation MKGAFilterByRawDataCell

+ (MKGAFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKGAFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGAFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKGAFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGAFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
#pragma mark -
- (void)textFieldValueChanged:(NSString *)text textType:(mk_ga_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(ga_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate ga_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGAFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_ga_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ga_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ga_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ga_filterRawAdvDataTextTypeRawDataType];
        };
    }
