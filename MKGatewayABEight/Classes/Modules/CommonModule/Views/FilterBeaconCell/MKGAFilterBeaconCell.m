...
@implementation MKGAFilterBeaconCellModel
@end

@interface MKGAFilterBeaconCell ()
...
@implementation MKGAFilterBeaconCell

+ (MKGAFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKGAFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGAFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKGAFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGAFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKGAFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGAFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ga_beaconMinValueChanged:index:)]) {
                [self.delegate mk_ga_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ga_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_ga_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
