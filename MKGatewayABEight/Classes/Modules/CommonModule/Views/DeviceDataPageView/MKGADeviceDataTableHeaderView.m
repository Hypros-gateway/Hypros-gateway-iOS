//
//  MKGADeviceDataTableHeaderView.m
...
@implementation MKGADeviceDataTableHeaderViewModel
@end

@interface MKGADeviceDataTableHeaderView ()
...
@implementation MKGADeviceDataTableHeaderView
...
- (void)uploadButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ga_updateLoadButtonAction)]) {
        [self.delegate ga_updateLoadButtonAction];
    }
}

- (void)scannerButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ga_scannerStatusChanged:)]) {
        [self.delegate ga_scannerStatusChanged:!self.scannerButton.selected];
    }
}

- (void)saveButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ga_saveButtonAction)]) {
        [self.delegate ga_saveButtonAction];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGADeviceDataTableHeaderViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGADeviceDataTableHeaderViewModel.class]) {
        return;
    }
...
- (void)reloadSubViews {
    UIImage *image = (self.scannerButton.selected ? LOADICON(@"MKGatewayABEight", @"MKGADeviceDataTableHeaderView", @"ga_switchSelectedIcon.png") : LOADICON(@"MKGatewayABEight", @"MKGADeviceDataTableHeaderView", @"ga_switchUnselectedIcon.png"));
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ga_scannerTimeChanged:)]) {
                [self.delegate ga_scannerTimeChanged:text];
            }
        };
