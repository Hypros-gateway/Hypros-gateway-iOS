...
static NSString *const mk_ga_logName = @"mk_ga_bleLog";

NSString *const mk_ga_peripheralConnectStateChangedNotification = @"mk_ga_peripheralConnectStateChangedNotification";
NSString *const mk_ga_centralManagerStateChangedNotification = @"mk_ga_centralManagerStateChangedNotification";

NSString *const mk_ga_deviceDisconnectTypeNotification = @"mk_ga_deviceDisconnectTypeNotification";

static MKGACentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKGACentralManager ()
...
@property (nonatomic, assign)mk_ga_centralConnectStatus connectStatus;

@end

@implementation MKGACentralManager

- (void)dealloc {
    [self logToLocal:@"MKGACentralManager销毁"];
    NSLog(@"MKGACentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKGACentralManager初始化"];
...
+ (MKGACentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKGACentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_ga_receiveDevice:)]) {
                [self.delegate mk_ga_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_ga_startScan)]) {
        [self.delegate mk_ga_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_ga_stopScan)]) {
        [self.delegate mk_ga_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_ga_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_ga_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_ga_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_ga_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_ga_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_ga_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_ga_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_ga_centralManagerStatus )centralStatus {
    ...
    ? mk_ga_centralManagerStatusEnable
    : mk_ga_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_ga_taskOperationID)operationID
    ...
    MKGAOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_ga_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKGAOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKGAPeripheral *trackerPeripheral = [[MKGAPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKGAOperation *operation = [[MKGAOperation alloc] initOperationWithID:mk_ga_connectPasswordOperation commandBlock:^{
        __strong typeof(self) sself = weakSelf;
        NSLog(@"设备连接密码:%@",sself.password);
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.ga_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_ga_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_ga_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKGAOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_ga_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKGAOperation <MKBLEBaseOperationProtocol>*operation = [[MKGAOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKGAOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_ga_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_ga_logName dataList:@[string]];
}
