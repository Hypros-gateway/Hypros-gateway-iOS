...
typedef NS_ENUM(NSInteger, mk_ga_connectMode) {
    mk_ga_connectMode_TCP,                                          //TCP
    mk_ga_connectMode_CASignedServerCertificate,                    //SSL.Do not verify the server certificate.
    mk_ga_connectMode_CACertificate,                                //SSL.Verify the server's certificate
    mk_ga_connectMode_SelfSignedCertificates,                       //SSL.Two-way authentication
};

//Quality of MQQT service
typedef NS_ENUM(NSInteger, mk_ga_mqttServerQosMode) {
    mk_ga_mqttQosLevelAtMostOnce,      //At most once. The message sender to find ways to send messages, but an accident and will not try again.
    mk_ga_mqttQosLevelAtLeastOnce,     //At least once.If the message receiver does not know or the message itself is lost, the message sender sends it again to ensure that the message receiver will receive at least one, and of course, duplicate the message.
    mk_ga_mqttQosLevelExactlyOnce,     //Exactly once.Ensuring this semantics will reduce concurrency or increase latency, but level 2 is most appropriate when losing or duplicating messages is unacceptable.
};

@interface MKGAInterface (MKGAConfig)
